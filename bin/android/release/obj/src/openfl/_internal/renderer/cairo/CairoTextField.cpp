...
HXLINE( 262)								cairo->textPath(text.substring(group->startIndex,group->endIndex));
...
HXLINE( 291)							cairo->showText(text.substring(group->startIndex,group->endIndex));
            						}
...
HXDLIN( 310)									cairo->moveTo(_hx_tmp17,(group->offsetY + ((Float)0.5)));
HXLINE( 311)									cairo->set_lineWidth((int)1);
HXLINE( 312)									Float _hx_tmp18 = (::Math_obj::floor((group->offsetX + advance)) + ((Float)0.5));
HXDLIN( 312)									cairo->lineTo(_hx_tmp18,((group->offsetY + group->height) - (int)1));
...
HXLINE( 338)									if ((selectionEnd >= textEngine->text.length)) {
HXLINE( 340)										end = textField->getCharBoundaries((textEngine->text.length - (int)1));
...
HXLINE( 359)										cairo->showText(text.substring(selectionStart,selectionEnd));
            									}
            								}
            							}
            						}
